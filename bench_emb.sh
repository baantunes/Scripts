...
calc_disk() {
    ...
    do
        ...
        #[ "`echo ${size:(-1)}`" == "M" ] && size=$( awk 'BEGIN{printf "%.1f", '$size_t'}' )
        [ "`echo ${size:(-1)}`" == "G" ] && size=$( awk 'BEGIN{printf "%.1f", '$size_t' * 1024}' )
        [ "`echo ${size:(-1)}`" == "M" ] && size=${size_t}
        total_size=$( awk 'BEGIN{printf "%.1f", '$total_size' + '$size'}' )
    done
    echo ${total_size}
}
hname=$(uci get system.@system[0].hostname)
cname=$( awk -F: '/system type/ {name=$2} END {print name}' /proc/cpuinfo | sed 's/^[ \t]*//;s/[ \t]*$//' )
cores=$( awk -F: '/system type/ {core++} END {print core}' /proc/cpuinfo )
freq=$( dmesg | grep "CPU clock"  | awk '{print $5}')
...
kern=$( uname -r )
disk_size1=($( df -hP | grep -wvE '\-|none|tmpfs|devtmpfs|by-uuid|chroot|Filesystem|udev|docker' | awk '{print $2}' ))
disk_size2=($( df -hP | grep -wvE '\-|none|tmpfs|devtmpfs|by-uuid|chroot|Filesystem|udev|docker' | awk '{print $3}' ))
...
echo -e "Total size of Disk   : ${BLUE}$disk_total_size MB ($disk_used_size MB Used)${PLAIN}"
